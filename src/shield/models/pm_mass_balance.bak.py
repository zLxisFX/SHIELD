...
DEFAULTS: Dict[str, PMArchetypeDefaults] = {
    "apartment": PMArchetypeDefaults(
        ...
        pen_open=0.95,
        k_dep_h=0.18,
        cadr_hepa_per_m2_m3_h=4.0,  # 4 m3/h per m2 (placeholder)
    ),
...
def defaults_for_archetype(archetype: str, floor_area_m2: float, has_hepa: bool) -> PMModelParams:
    a = str(archetype).lower()
    if a not in DEFAULTS:
        a = "classroom"

    d = DEFAULTS[a]
    volume = float(d.volume_per_m2_m3 * float(floor_area_m2))

    cadr = float(d.cadr_hepa_per_m2_m3_h * float(floor_area_m2)) if bool(has_hepa) else 0.0
...
def make_params(building: BuildingProfile) -> PMModelParams:
    return defaults_for_archetype(
        archetype=building.archetype,
        floor_area_m2=float(building.floor_area_m2),
        has_hepa=bool(building.has_hepa),
    )
